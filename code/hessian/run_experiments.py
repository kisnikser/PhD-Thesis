...
from code.hessian.spectra import gn_matvec, hessian_matvec, power_iteration
...
def _theoretical_bound(model, x, L):
    M_x = float(x.view(x.shape[0], -1).norm(dim=1).max().item())
    M_W = 0.0
    for m in model.modules():
        if isinstance(m, torch.nn.Linear):
            M_W = max(M_W, float(m.weight.norm(2).item()))
...
def compute_spectral_norms(model, x, y, device, dtype, num_iters=30, tol=1e-4):
    ...
    L = sum(1 for m in model.modules() if isinstance(m, torch.nn.Linear))

    def h_matvec(v):
        return hessian_matvec(F.cross_entropy, model, x, y, v)

    def g_matvec(v):
        return gn_matvec(model, x, y, v)
    ...
    bound = _theoretical_bound(model, x, L)
...
def main(conf=None):
    if conf is None:
        conf_path = Path(__file__).parent / "experiments_config.yaml"
    ...
    out_path = out_dir / "hessian_experiments.json"
