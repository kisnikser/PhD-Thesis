...
\nomenclature[21b]{$\mathbf{w}_{m}^{*} \in \mathbb{W}$}{точка локального минимума функции потерь на выборке~$\mathfrak{D}_m$}
\nomenclature[26a]{$\mathbf{G}^{(p, q)}(\mathbf{w}) \in \mathbb{R}^{N_p \times N_q}$}{блок гаусс-ньютоновской матрицы, соответствующий параметрам $p$-го и $q$-го слоев}
\nomenclature[27b]{$n_p$}{размерность пространства предактивации $p$-го слоя}
\nomenclature[27a]{$N_p$}{число параметров $p$-го слоя}
\nomenclature[28b]{$\vecop(\cdot)$}{оператор векторизации}
\nomenclature[28с]{$\vecrop(\cdot)$}{оператор векторизации (по строкам)}
\nomenclature[35a]{$\lambda_i$}{собственное значение матрицы Гессе}
\nomenclature[35b]{$\mathbf{u}_i \in \mathbb{R}^{N}$}{собственный вектор матрицы Гессе}
\nomenclature[36a]{$\|\cdot\|_{2}$}{евклидова норма вектора и соответствующая ей спектральная норма матрицы}
