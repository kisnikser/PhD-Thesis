...
In this paper, we present two approaches for determining sufficient sample sizes based on the distance between posterior distributions of model parameters. Our methods involve comparing the posterior distributions over two similar subsamples. If the sample size is adequate, these posterior distributions should be close to each other. To quantify this proximity, we propose using the Kullback-Leibler (KL) divergence~\cite{MOTRENKO2014743} and the s-score model comparison function~\cite{Aduenko2017}. We also encourage readers to explore our experimental code\footnote{\href{https://github.com/kisnikser/Posterior-Distributions-Proximity}{https://github.com/kisnikser/Posterior-Distributions-Proximity}} for a deeper understanding of our methods and results.

The key contributions of our work are as follows:
\begin{enumerate}
    \item \textbf{Methodological Innovation:} We introduce two novel methods for assessing the sufficiency of sample sizes by examining the proximity of posterior distributions. These methods leverage the KL-divergence and the s-score similarity function to provide interpretable results. Comparing to existing approaches, our methods do not rely on the specific hypothesis about the model parameters as the statistical ones, and have a proved correctness in contrast to heuristic techniques. 
    \item \textbf{Theoretical Validation:} We prove the correctness of our proposed methods. Specifically, we demonstrate that for a given model, the respective functions converge to zero as the sample size increases. This convergence provides a solid foundation for selecting an appropriate sample size based on the descent of these functions. Our proofs are established within a probabilistic framework assuming a normal posterior distribution of parameters. Additionally, for the linear regression model, we prove a theorem regarding the moments of the limit posterior distribution of parameters.
    \item \textbf{Empirical Analysis:} Through a detailed empirical study, we compare our methods with existing approaches. This comparison highlights the unique characteristics of our proposed methods. Notably, we find that the KL-sufficient sample size is typically the largest, while the s-score sufficient sample size is the smallest. This disparity arises from the different functions used to compare probability distributions.
\end{enumerate}
...
As in the case of a KL-sufficient sample size, in a model with a normal posterior distribution, it is possible to write an expression for the criterion used. Thus, one more statement can be formulated. The proof is again provided in the Appendix~\ref{app:theorem2}.
...
The second condition of Theorem~\ref{theorem3} is of greater interest, as it delves into the behavior of the minimum eigenvalue of the sample covariance matrix of features. We denote $\omega(\sqrt{k})$ as a growth rate that is asymptotically faster than $\sqrt{k}$, ensuring that the smallest eigenvalue of $\mathbf{X}\T_k \mathbf{X}_k$ does not decay too quickly. Unfortunately, this paper does not provide theoretical guarantees for this convergence, although it is verified through experiments. This can be regarded as a limitation, so we will try to reveal this in the future work.
...
\begin{figure}[ht]
    ...
    \caption{Minimum eigenvalue versus the available sample set size. The estimated graph, which is blue, for large sample sizes is higher than the orange graph of the power function. This asymptotic behavior of minimum eigenvalue corresponds to the second assumption in the Theorem~\ref{theorem3}.}
    \label{eigvals}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{dependence_on_available_sample_set.pdf}
...
In this paper, we proposed methods for determining sufficient sample sizes by examining the proximity of posterior distributions of model parameters across subsamples. We proved the validity of these methods under certain model restrictions and demonstrated their effectiveness through computational experiments. A key assumption in our work was that the posterior distribution of model parameters is gaussian. This assumption allowed us to significantly simplify the theoretical calculations, enabling us to derive explicit formulas for the Kullback-Leibler divergence and the s-score similarity function. However, this assumption also limits the generalizability of our results. Future research will need to explore scenarios with non-normal posterior distributions to address this limitation.

In our experiments, we used the proposed methods in a relative manner, calculating the relative deviation from the value on the entire dataset. We suggest adopting this approach in practice, with the choice of the threshold left to the researcher. We recommend using small values, such as $0.05$ or $0.1$, for practical applications.

A potential challenge in applying our methods lies in the calculation of the Kullback-Leibler divergence and the s-score similarity function, both of which involve the inversion of the covariance matrix. The computational complexity of this operation increases with the number of features in the dataset, making these approaches less suitable for very large models, particularly neural network models. We plan to develop research in this direction to address these computational challenges.

Regarding the comparison between KL-divergence and s-score similarity, we discussed this issue in detail in the experiments section. If a more conservative estimate is desired, we recommend using KL-divergence. Conversely, if a more optimistic estimate is required, the s-score similarity function is more appropriate.

We hope that our results will serve as a foundation for further research in this area, particularly in the context of more complicated machine learning models. Our future work will aim to relax the assumption of a gaussian posterior distribution and explore alternative methods to handle the computational complexities associated with large-scale models.

\newpage
\begin{appendices}

\section{Proof of Theorem \ref{theorem1}}\label{app:theorem1}
\begin{proof}
...
\section{Proof of Theorem \ref{theorem2}}\label{app:theorem2}
\begin{proof}
...
\section{Proof of Theorem \ref{theorem3}}\label{app:theorem3}
\begin{proof}
